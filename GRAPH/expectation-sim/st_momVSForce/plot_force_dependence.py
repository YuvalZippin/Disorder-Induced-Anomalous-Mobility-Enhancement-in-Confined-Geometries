...
def report(df, alpha, T, A):
    ...
    print("=" * 96)
    hdr = ("  lattice   w  Omega        F      eps_th     eps_sim    Lam_th    Lam_sim"
           "   exact/asym    sim/thy")
    print(hdr)
    print("  " + "-" * 94)
    ...
    for (lat, w), g in df.groupby(["lattice", "w"], sort=False):
        ...
        for _, r in g.iterrows():
            ...
            ratio_sim = r["mean_x"] / x_theory_asymptotic(r["F"], r["Omega"], r["q"],
                                                          alpha, T, A)
            print("  %-7s %3d %6d  %9.2e  %9.2e  %9.2e  %8.2e  %8.2e   %8.4f   %8.4f"
                  % (lat, w, r["Omega"], r["F"], r["eps_theory"],
                     r.get("esc_meas", np.nan), lam_th, r.get("lambda_meas", np.nan),
                     ratio_exact, ratio_sim))
        print("  " + "-" * 94)

    eps_max = df["eps_theory"].max()
    print("  max eps in the data set : %.4f" % eps_max)
    if eps_max > 0.10:
        print("  ** WARNING ** eps > 0.1: the nearly recurrent expansion is no longer safe.")
    elif eps_max > 0.06:
        print("  note: eps above 0.06; the prefactor error approaches a few per cent.")
    else:
        print("  the whole data set sits inside the nearly recurrent regime.")
