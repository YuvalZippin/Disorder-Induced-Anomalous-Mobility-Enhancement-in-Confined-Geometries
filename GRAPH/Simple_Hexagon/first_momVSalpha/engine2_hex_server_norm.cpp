// engine2_hex_server_norm.cpp
// PRO-Grade Subordination Engine - Server Edition (64 Threads)
// Geometry B: Simple Hexagonal Lattice (Effective 1D mapping of Reflecting Boundaries)
// Compile: g++ -std=c++17 -O3 -march=broadwell -fopenmp -o engine2_hex_server_norm engine2_hex_server_norm.cpp
// Run:     export OMP_NUM_THREADS=64 && ./engine2_hex_server_norm
...
int main() {
    ...
    std::cout << "[SYSTEM] Initializing Engine 2 Hexagonal (Server Edition) on " << max_threads << " threads.\n";
    ...
    for (const double alpha : Config::ALPHAS) {
        ...
        for (const int w : Config::WIDTHS) {
            ...
            // 2. Effective force mapping using the hexagonal area
            const double F_eff = Config::KAPPA_ALPHA * std::pow(Config::F_PHYS, alpha) * std::pow(omega, alpha - 1.0);
            
            // 3. Lattice transitions: 2 longitudinal + 6 transverse neighbors = 8
            const double norm_factor_prob = 1.0 / (2.0 * std::cosh(F_eff / 2.0) + 6.0);
            
            const double P_plus = norm_factor_prob * std::exp(F_eff / 2.0);
            const double P_minus = norm_factor_prob * std::exp(-F_eff / 2.0);
            ...
            // D_0 updated for hexagonal lattice (NN=8) -> 1/8
            const double D_0 = 1.0 / 8.0; 
