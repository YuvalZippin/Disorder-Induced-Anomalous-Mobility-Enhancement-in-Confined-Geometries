#!/usr/bin/env python3
# plot_graph_b_exact.py
# Usage: python3 plot_graph_b_exact.py --csv results_geometry_a.csv --alpha 0.3 --out graph_b_width_dependence.png
...
def main():
    ap = argparse.ArgumentParser(description="Plot exact analytical scaling vs Simulation for Geometry A (Width)")
    ap.add_argument("--csv", required=True, default="results_geometry_a.csv", help="Input CSV")
    ap.add_argument("--alpha", type=float, default=0.3, help="Anomalous exponent")
    ap.add_argument("--out", default="graph_b_width_dependence.png", help="Output filename")
    args = ap.parse_args()
    ...
    print(f"[SYSTEM] Expected theoretical scaling: <x> ~ w^({theoretical_power:.2f})")
