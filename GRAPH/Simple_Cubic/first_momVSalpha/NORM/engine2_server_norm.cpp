...
namespace Config {
    ...
    constexpr double T_TARGET = 1e5;            
    constexpr double F_PHYS = 0.05;              
    constexpr double KAPPA_ALPHA = 10.0;         
    constexpr uint64_t MASTER_SEED = 0x9e3779b97f4a7c15ULL;
    
    // BUG FIX: Array size matched to initialized values to prevent zero-padding and NaN crash
        ...
        for (const int w : Config::WIDTHS) {
            ...
            #pragma omp parallel
            {
                ...
                for (uint64_t n = 0; n < Config::N_TRAJ; ++n) {
                    ...
                    // ALGORITHMIC LEAP: O(1) Binomial sampling instead of O(N) microscopic step loop
