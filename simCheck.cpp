...
public:
    ...
    static double measure_A_coefficient(const std::vector<double>& t_values, 
        ...
        if (fit.success && fit.r_squared > 0.8) {  // Good fit threshold
            std::cout << " A=" << std::scientific << std::setprecision(3) << fit.A 
                      << " (R²=" << std::fixed << std::setprecision(3) << fit.r_squared << ")" << std::endl;
            return fit.A;
        } else {
            std::cout << " FAILED FIT (R²=" << std::fixed << std::setprecision(3) 
                      << fit.r_squared << ")" << std::endl;
    ...
    static void analyze_A_vs_system_size(int L_start, int L_finish, int L_step,
        ...
        file << "System_Size_L,Mean_A_Coefficient,Std_A_Coefficient,Num_Repetitions,Valid_Fits\n";
        ...
        for (int L = L_start; L <= L_finish; L += L_step) {
            ...
            if (!A_values.empty()) {
                ...
                file << L << ",NaN,NaN," << num_repetitions << ",0\n";
            }
        }
...
int main() {
    ...
    int L_start = 5;          // Starting system size (avoid L=0 for now)
    int L_finish = 100;       // Ending system size
    int L_step = 25;          // Step size for system sizes (gives us 4 points: 5,30,55,80)
    
    // Analysis parameters
    int num_repetitions = 3;  // Number of repetitions per L for averaging
