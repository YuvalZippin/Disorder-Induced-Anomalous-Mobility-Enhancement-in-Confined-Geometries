...
//
//  Build:
//    g++ -std=c++17 -O3 -march=native -fopenmp -DTCBITS=15 \
...
#ifndef TCBITS
#define TCBITS 15          // log2 of tau-cache slots per thread (2^15 * 16B = 512 kB)
#endif
...
//  no reset is needed.
// ---------------------------------------------------------------------
//  V3 change: the cache is indexed POSITIONALLY, by (x mod 2^xb, y), not by
//  the hash of the site. The hash is still computed and stored as the key, so
//  tau values are bit-identical to V1/V2 -- only the slot choice differs.
//  Why it is better: the walk is ballistic in x, so a positional index gives
//  a small sliding hot window that stays in L1/L2, whereas a hashed index
//  scatters over the whole table. Wrap-around costs nothing, because a
//  forward-drifting walker never comes back to an overwritten column.
//  Measured: the miss rate drops to ~eps, i.e. exactly one pow() per distinct
//  site, which is the theoretical floor.
struct alignas(64) TauCache {
    ...
    void init(int w_, int logent) {                 // logent = log2(total slots)
        w = w_;
        int sh = 0; while ((1 << sh) < w) ++sh;      // ceil(log2 w)
        int xb = logent - sh; if (xb < 4) xb = 4;
        xmask = (size_t(1) << xb) - 1;
        const size_t n = (xmask + 1) * (size_t)w;
        key.assign(n, ~0ULL); tau.assign(n, 0.0);
    }
};
...
static inline double tau_of_site(TauCache& C, int64_t x, int32_t y, uint64_t dseed,
    ...
    const size_t i = ((size_t)((uint64_t)x & C.xmask)) * (size_t)C.w + (size_t)y;
    if (C.key[i] == h) { ++C.hits; return C.tau[i]; }
    const double u = (double)((h >> 11) + 1) * 0x1.0p-53;
    const double v = scale * std::pow(u, neg_inv_alpha);
    C.key[i] = h; C.tau[i] = v; ++C.misses;
    return v;
}

//  Branchless step table. The four-way choice is data-dependent and
//  unpredictable, so an if/else chain mispredicts on most steps; replacing it
//  with three compares and two table lookups measured ~1.55x faster overall.
static const int STEP_DX[4] = { 1, -1, 0, 0 };
static const int STEP_DY[4] = { 0,  0, 1, -1 };
                     ...
                     std::vector<int64_t>& xrec, std::vector<uint64_t>& Nrec, bool& capped)
{
    ...
    for (int ti = 0; ti < K; ) {
        ...
        const int dir = (int)(r >= c1) + (int)(r >= c2) + (int)(r >= c3);
        x += STEP_DX[dir];
        int32_t yn = y + STEP_DY[dir];
        if (P.bc == BC_PBC) { yn = (yn == w) ? 0 : yn; yn = (yn < 0) ? w-1 : yn; }
        else                { yn = (yn >= w || yn < 0) ? y : yn; }
        y = yn;
        ++N;
...
// ---------------------------------------------------------------------
//  V3 addition: PAIRED bootstrap of the Omega-exponent.
//  With common random numbers, walker n at Omega_1 and walker n at Omega_2
//  share their random stream and their disorder, so the per-Omega means are
//  strongly positively correlated. The independent per-point error bars used
//  by fit_loglog therefore OVERSTATE the uncertainty of the slope.
//  Here one bootstrap replicate resamples the walker INDEX once, rebuilds
//  every Omega mean from that same resampled set, and refits. The spread of
//  the resulting slopes is the honest CI. This costs no extra simulation.
// ---------------------------------------------------------------------
struct BootSlope { double slope, lo, hi, se; int nrep; };

static BootSlope paired_slope(const std::vector<double>& wvals,
                              const std::vector<std::vector<double>>& xw,
                              int nboot, uint64_t seed)
{
    BootSlope B{NAN,NAN,NAN,NAN,0};
    const size_t M = wvals.size();
    if (M < 2 || xw.size() != M || xw[0].empty()) return B;
    const size_t NW = xw[0].size();
    std::vector<double> lw(M);
    for (size_t i = 0; i < M; ++i) lw[i] = std::log(wvals[i]);

    auto ols = [&](const std::vector<double>& ly) {
        double sx=0, sy=0, sxx=0, sxy=0; const double n=(double)M;
        for (size_t i=0;i<M;++i){ sx+=lw[i]; sy+=ly[i]; sxx+=lw[i]*lw[i]; sxy+=lw[i]*ly[i]; }
        return (n*sxy - sx*sy) / (n*sxx - sx*sx);
    };

    std::vector<double> ly(M);
    for (size_t i=0;i<M;++i){
        long double s=0.0L; for (double v : xw[i]) s+=v;
        const double m=(double)(s/(long double)NW);
        if (!(m>0.0)) return B;
        ly[i]=std::log(m);
    }
    B.slope = ols(ly);
    ...
    rep.reserve((size_t)std::max(1,nboot));
    Rng r; r.seed(seed);
    std::vector<size_t> idx(NW);
    for (int t=0;t<nboot;++t){
        for (size_t j=0;j<NW;++j) idx[j] = (size_t)(r.next() % NW);
        bool good = true;
        for (size_t i=0;i<M;++i){
            long double s=0.0L;
            const std::vector<double>& col = xw[i];
            for (size_t j=0;j<NW;++j) s += col[idx[j]];
            const double m=(double)(s/(long double)NW);
            if (!(m>0.0)) { good=false; break; }
            ly[i]=std::log(m);
        }
        if (good) rep.push_back(ols(ly));
    }
    if (rep.size() < 8) return B;
    std::sort(rep.begin(), rep.end());
    B.nrep = (int)rep.size();
    B.lo = rep[(size_t)(0.025*rep.size())];
    B.hi = rep[(size_t)std::min<double>(rep.size()-1, 0.975*rep.size())];
    long double s=0.0L, ss=0.0L;
    for (double v : rep){ s+=v; ss+=(long double)v*v; }
    const double mu=(double)(s/(long double)rep.size());
    B.se = std::sqrt(std::max(0.0,(double)(ss/(long double)rep.size()) - mu*mu));
    return B;
}
...
int main(int argc, char** argv) {
    ...
    for (double f : Fl) {
        ...
        std::vector<std::vector<double>> xw(wl.size());   // per-Omega, per-walker x at Tmax
            ...
            for (int k=0;k<K;++k) {
                ...
                if (k == K-1) {
                    rows.push_back(Row{P.F,(double)P.w,Tl[k],S.mean,S.se,S.ci_lo,S.ci_hi,
                                       Nm,th.eps,rat,ok});
                    size_t widx = 0;
                    for (size_t q=0;q<wl.size();++q) if ((int)std::llround(wl[q]) == P.w) widx = q;
                    xw[widx].assign(xall.begin()+(size_t)k*NW, xall.begin()+(size_t)(k+1)*NW);
                }
            }
        ...
        const BootSlope B = paired_slope(wl, xw, NBOOT, mix64(MSEED ^ (uint64_t)(f*1e9) ^ 0x51EDULL));
        if (B.nrep > 0)
            std::printf("# paired bootstrap over walkers (CRN-aware): slope = %.4f  se = %.4f"
                        "  95%% CI [%.4f, %.4f]  (%d reps)\n",
                        B.slope, B.se, B.lo, B.hi, B.nrep);
    }
