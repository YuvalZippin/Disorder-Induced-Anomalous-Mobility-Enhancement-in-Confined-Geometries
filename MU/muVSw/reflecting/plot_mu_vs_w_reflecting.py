#!/usr/bin/env python3
# reflecting/plot_mu_vs_w_reflecting.py
# Usage:
#   python3 plot_mu_vs_w_reflecting.py --csv results_reflecting.csv --alpha 0.3 --auto-K
#   python3 plot_mu_vs_w_reflecting.py --csv results_reflecting.csv --alpha 0.3 --K 0.19
import argparse, math
import numpy as np, pandas as pd, matplotlib.pyplot as plt

def main():
    ap = argparse.ArgumentParser(description="Reflecting BC: plot mu(w)=<x>/(F^alpha t^alpha) vs w with theory overlay.")
    ...
    ap.add_argument("--K", type=float, default=None, help="K_lead amplitude; if omitted and --auto-K set, estimate from C_sim plateau")
    ...
    ap.add_argument("--out", default="reflecting_mu_vs_w.png", help="Output PNG filename")
    ...
    # K_lead from C_sim tail (reflecting amplitude)
    K_used = args.K
    if K_used is None and args.auto_K:
        ...
        print(f"[auto-K] Estimated K_lead (reflecting) ≈ {K_used:.6g}" if K_used is not None else "[auto-K] Could not estimate K")

    # plot
    plt.figure(figsize=(6.8, 4.6))
    plt.errorbar(ws, mu_w, yerr=mu_w_err, fmt="o", ms=5, lw=1.2, label="simulation (reflecting)")
    ...
    plt.xlabel("width w (reflecting y,z)")
    plt.ylabel("mu(w) = <x> / (F^alpha t^alpha)")
    plt.title("mu vs w (reflecting; expect slope -2(1-alpha))")
    plt.legend()
    plt.tight_layout()
