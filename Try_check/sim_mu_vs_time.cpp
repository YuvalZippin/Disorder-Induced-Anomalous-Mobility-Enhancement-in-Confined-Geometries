// sim_mu_vs_time.cpp
// Compile: g++ -std=c++17 -O3 -pthread -o sim_mu_vs_time sim_mu_vs_time.cpp
// Run:     ./sim_mu_vs_time
...
struct SimConfig {
    ...
    int measurement_points = 5;      // log-spaced t points
    double t_min = 1e05;
    double t_max = 1e15;
...
int main() {
    ...
    out << "t,mu,average_x,steps_max,steps_mean\n";
    ...
    double F_use_nom = clamp_force_for_width(cfg.F_input, cfg.w, cfg.c_small_force);
    ...
    for (int m = 0; m < cfg.measurement_points; ++m) {
        ...
        uint64_t seed = cfg.base_seed ^ (uint64_t)cfg.w ^ (uint64_t)m;
        ...
        // mu(t) observable: <x> t^{-α} F^{-α} w^{2(1-α)}
        double mu_t = avg_x * std::pow(t, -cfg.alpha)
    ...
    cerr << "\nPlot mu_vs_time.csv to see when mu(t) plateaus (asymptotic regime).\n";
    return 0;
}
