// sim_ratio_only.cpp
// Compile: g++ -std=c++17 -O3 -pthread -o sim_ratio_only sim_ratio_only.cpp -lgsl -lgslcblas
// Run:     ./sim_ratio_only
...
int main() {
    ...
    out << "t,ratio_sim,steps_max,steps_mean\n";
    ...
    for (int m = 0; m < cfg.measurement_points; ++m) {
        ...
        // Only compute analytic_full and output R(t)
        double analytic_full = moment_eq20(cfg.F, cfg.Q0, cfg.alpha, t, cfg.A);
        double R_sim = sim_moment / analytic_full;

        out << std::setprecision(17) << t << ","
            << std::setprecision(17) << R_sim << ","
            << steps_max << ","
    ...
    cerr << "\nCSV written: " << cfg.csv << "\n";
    cerr << "Plot in Python with matplotlib for ratio_sim vs time.\n";
    return 0;
}
