// sim_and_analytic_ratio.cpp
// Compile: g++ -std=c++17 -O3 -pthread -o sim_and_analytic_ratio sim_and_analytic_ratio.cpp -lgsl -lgslcblas
// Run:     ./sim_and_analytic_ratio
...
struct SimConfig {
    ...
    int w = 5;
    double A = 10.0;
    double Q0 = 0.339499; // Use your numerically evaluated value
    uint64_t base_seed = 0x9e3779b97f4a7c15ULL;
    string csv = "sim_and_analytic_ratio.csv";
};

// ---------------------- Math helpers ----------------------
inline double gamma1p(double a) { return gsl_sf_gamma(1.0 + a); }
// Hardcoded polylog value from Python
const double polylog_val_hardcoded = 0.5673127065079584;
...
double moment_eq20(double F, double Q0, double alpha, double t, double A) {
    ...
    double gamma1p_alpha = gamma1p(alpha);
    // Use the hardcoded polylog value
    double factor = Q0 / (A * gamma1p_alpha * std::pow(1.0 - Q0, 2.0) * polylog_val_hardcoded);
...
int main() {
    ...
    out << "t,sim_moment,analytic_leading,analytic_full,ratio_sim,ratio_analytic,steps_max,steps_mean\n";

    cout << "w=" << cfg.w << "  F=" << cfg.F << "  alpha=" << cfg.alpha << "  Q0=" << cfg.Q0 << "\n";
    ...
    for (int m = 0; m < cfg.measurement_points; ++m) {
        ...
        // Analytical leading and full moments
        double analytic_leading = moment_eq34(cfg.F, cfg.w, cfg.alpha, t, cfg.A);
        double analytic_full = moment_eq20(cfg.F, cfg.Q0, cfg.alpha, t, cfg.A);

        // Ratio from simulation and analytic
        double ratio_sim = sim_moment / analytic_full;
        double ratio_analytic = analytic_leading / analytic_full;

        out << std::setprecision(17) << t << ","
            << std::setprecision(17) << sim_moment << ","
            << std::setprecision(17) << analytic_leading << ","
            << std::setprecision(17) << analytic_full << ","
            << std::setprecision(17) << ratio_sim << ","
            << std::setprecision(17) << ratio_analytic << ","
            << steps_max << ","
            << std::setprecision(17) << avg_steps << "\n";

        cout << "t=" << std::scientific << t
             << "  sim_moment=" << sim_moment
             << "  analytic_leading=" << analytic_leading
    ...
    cerr << "\nCSV written: " << cfg.csv << "\n";
    cerr << "Plot in Python with matplotlib for ratio_sim vs ratio_analytic vs time.\n";
    return 0;
}
