// sim_and_analytic_moment_eq20.cpp
// Compile: g++ -std=c++17 -O3 -pthread -o sim_and_analytic_moment_eq20 sim_and_analytic_moment_eq20.cpp -lgsl -lgslcblas
// Run:     ./sim_and_analytic_moment_eq20
...
struct SimConfig {
    ...
    string csv = "sim_and_analytic_moment_eq20.csv";
};

// ---------------------- Math helpers ----------------------
inline double gamma1p(double a) { return std::tgamma(1.0 + a); }

// Simple polylog(s,z) series implementation valid for |z| < 1:
//   Li_s(z) = sum_{k=1..inf} z^k / k^s
// This is sufficient for the use here where Q0 < 1; it uses a
// convergence tolerance and a maximum iteration guard.
inline double polylog(double s, double z) {
    if (!(std::isfinite(z)) || std::abs(z) >= 1.0) {
        // fallback: Q0 should be < 1 in typical runs; return NaN for unsupported inputs
        return std::numeric_limits<double>::quiet_NaN();
    }
    const int max_iter = 1000000;
    const double tol = 1e-14;
    double sum = 0.0;
    double zk = z; // z^k with k=1 initially
    for (int k = 1; k <= max_iter; ++k) {
        // term = z^k / k^s
        double term = zk / std::pow((double)k, s);
        sum += term;
        if (k > 10 && std::abs(term) < tol * std::abs(sum)) break;
        zk *= z;
    }
    return sum;
}
double compute_Q0_3D(int w, double F) {
    double B = 1.0 / (2.0 * std::cosh(F / 2.0) + 4.0);
    double sum = 0.0;
    for (int m = 0; m < w; ++m) {
        double km = 2.0 * M_PI * m / w;
        double cos_km = std::cos(km);
        for (int n = 0; n < w; ++n) {
            double kn = 2.0 * M_PI * n / w;
            double cos_kn = std::cos(kn);
            double S = cos_km + cos_kn;
            double term = std::pow(1.0 - 2.0 * B * S, 2.0) - 4.0 * B * B;
            sum += 1.0 / std::sqrt(term);
        }
    }
    double Q0 = 1.0 - (w * w) / sum;
    return Q0;
}

// ---------------------- Eq. 20 moment ----------------------
double moment_eq20(double F, double Q0, double alpha, double t, double A) {
    double num = std::sinh(F / 2.0);
    double denom = 2.0 + std::cosh(F / 2.0);
    double gamma1p_alpha = gamma1p(alpha);
    double polylog_val = polylog(-alpha, Q0);
    double factor = Q0 / (A * gamma1p_alpha * std::pow(1.0 - Q0, 2.0) * polylog_val);
    return (num / denom) * factor * std::pow(t, alpha);
}
...
int main() {
    ...
    // Compute Q0 once for all t (since F, w fixed)
    double Q0 = compute_Q0_3D(cfg.w, cfg.F);

    ofstream out(cfg.csv);
    out << "t,sim_moment,analytic_moment,ratio,steps_max,steps_mean\n";

    cout << "w=" << cfg.w << "  F=" << cfg.F << "  alpha=" << cfg.alpha << "  Q0=" << Q0 << "\n";
    ...
    for (int m = 0; m < cfg.measurement_points; ++m) {
        ...
        // Analytical moment
        double analytic_moment = moment_eq20(cfg.F, Q0, cfg.alpha, t, cfg.A);
