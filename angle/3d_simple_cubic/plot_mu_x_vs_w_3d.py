#!/usr/bin/env python3
# plot_mu_x_vs_w_3d.py
#
# Example usage:
#   python3 plot_mu_x_vs_w_3d.py --csv results_3d_angled.csv --alpha 0.3 --auto-K --show
import argparse, math
import numpy as np, pandas as pd, matplotlib.pyplot as plt

def main():
    ap = argparse.ArgumentParser(description="Plot mu_x(w) with theory overlay for 3D channel QTM.")
    ap.add_argument("--csv", required=True, help="CSV with results_3d_angled.csv columns: w,t,F_used,theta_deg,phi_deg,average_x,C_sim_x, etc.")
    ...
    ap.add_argument("--out", default="mu_x_vs_w_3d.png", help="Output PNG filename")
    ap.add_argument("--show", action="store_true", help="Show figure interactively")
    args = ap.parse_args()

    df = pd.read_csv(args.csv)
    required = {"w","t","F_used","theta_deg","phi_deg","average_x","C_sim_x"}
    if not required.issubset(df.columns):
        raise ValueError(f"CSV must have columns: {required}")

    alpha = args.alpha
    # Convert angles to radians
    df["theta"] = np.deg2rad(df["theta_deg"].values)
    df["phi"] = np.deg2rad(df["phi_deg"].values)
    # Compute per-row normalized mu_x for 3D channel theory
    scale_factor = (df["F_used"] * np.cos(df["theta"]) * np.sin(df["phi"])) ** alpha * np.power(df["t"], alpha)
    df["mu_x"] = df["average_x"] / scale_factor
    ...
    plt.title(r"$\mu_x$ vs $w$ for 3D channel drive")
